...
function shortcode_button($atts, $content = null) {
	...
	return '<a class="external mc-button ' . $atts['size'] . ' ' . $shape . ' ' . $outline . ' ' . $color . '" href="' . $atts['link'] . '" target="' . $atts['target'] . '">' .do_shortcode($content). '</a>';
}
...
function shortcode_tagline_box($atts, $content = null) {
    ...
    if($atts['link'] && $atts['button']){
        ...
        $out .= '<a href="'.$atts['link'].'" target="' . $atts['target'] . '" class="mc-button medium grey move">'.$atts['button'].'</a>';
    }
...
function shortcode_homeurl( $atts, $content = null ) {  
	return get_home_url();
}

add_shortcode('backgroundurl', 'shortcode_backgroundurl');
function shortcode_backgroundurl( $atts, $content = null ) {  
	$atts = shortcode_atts(array('url' => ''),$atts);
	$url = $atts['url'];
	$homeurl = get_home_url();
	return "style='background: url(".get_home_url() . $atts['url'].")'";
}
...
function shortcode_pricing_footer($atts, $content = null) {
    ...
    $str = '';
    $str .= '<li><a class="mc-button medium grey rounded" href="' . $atts['url'] . '" target="_blank">' . do_shortcode( $content ) . '</a></li>';
...
function shortcode_recent_posts($atts, $content = null) {
    ...
    while ( $posts_query->have_posts()) {
        ...
        $str .= '<p class="recent-post-date"><span class="post-date">'.get_the_time('j').'</span><span class="post-month">'.get_the_time('M').'</span></p>';
        $str .= '<div class="blog-type">';
		$str .= '<div class="recent-post-content">';
        ...
        if($categories){
            ...
            foreach($categories as $category) {
                $str_categories .= '<a href="'.get_category_link( $category->term_id ).'" class="external">'.$category->cat_name.'</a>'.$separator;
            }

            $str_categories = trim($str_categories, $separator);
        }
		
		apply_filters('excerpt_length', $atts['post_content_length']);
        $author_id = get_the_author_meta("ID");
        $author_url = get_author_posts_url( $author_id );
        $str .= '<p class="blog-meta">'.get_the_time('j F Y').' - by <a href="' . $author_url . '" class="external">' . get_the_author() . '</a> <span class="post-meta">| ' . $str_categories . ' | <a href="' . get_comments_link() . '"> ' . get_comments_number() . ' comment(s) </a></span></p>';
		$str .= '<div class="blog-content">'.get_the_excerpt($atts['post_content_length']).'</div>';
        $str .= '</div>';
        $str .= '</div>';
        $str .= '</div>';
    }
...
function shortcode_visit_project($atts, $content = null) {
    ...
    $str .= '<a class="mc-button medium grey" href="' . $atts['url'] . '" target="_blank">' . $atts['button_name'] . '</a>';
    $str .= "</div>";
...
// Social Media
//////////////////////////////////////////////////////////////////
add_shortcode('custom_social', 'shortcode_social');
function shortcode_social($atts) {
		$atts = shortcode_atts(
			...
			$str = '';
			if( $global_theme_options['footer_social_links'] ) {
				$str .= '<ul class="socials-icons">';
				foreach( $social_links as $key => $value ){
					...
					if( $global_theme_options[$option_idx] ){
					   $str .=  '<li><a href="'. $global_theme_options[$option_idx] .'" class="block-icon"><i class="fa fa-'.$value.'"></i></a></li>';
					}
				}
				 $str .= '</ul>';
			}
...
function add_shortcode_plugins( $plugin_array ){
    ...
    $plugin_array['MCShortcodes'] = get_template_directory_uri() . '/components/shortcodes/shortcodes.js';
    return $plugin_array;
}

function register_shortcode_menu_buttons( $buttons ){

    array_push( $buttons, "|", 'mc_shortcode_button' );
    return $buttons;
}
