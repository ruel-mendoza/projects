	...
	jQuery(document).ready(function($) {
		...
		geocoder.geocode({'address':'<?php echo addslashes($global_theme_options['gmap_address']); ?>'}, function(results, status) {
			if(status == google.maps.GeocoderStatus.OK) {
				...
				$('#map_canvas').gmap( settings ).bind('init', function(ev, map) {
					$('#map_canvas').gmap('addMarker', {'position': latitude+','+longitude, 'bounds': true, icon : new google.maps.MarkerImage("<?php if( trim($global_theme_options["gmap_marker_icon"]) == "" ) echo get_template_directory_uri() . '/images/marker.png'; else echo $global_theme_options["gmap_marker_icon"]; ?>") }).click(function() {
						$('#map_canvas').gmap('openInfoWindow', {'content': contentString}, this);
					});
					$('#map_canvas').gmap('option', 'zoom', <?php if( $global_theme_options['map_zoom_level'] ) echo $global_theme_options['map_zoom_level']; else echo '8'; ?>);

				});

                var gmap = $('#map_canvas').gmap('get', 'map');
