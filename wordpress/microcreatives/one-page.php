...
if( have_posts() ) {
	...
	while ($main_query->have_posts()) {
    	...
    	$section_page = (get_post_meta($post_id, "mc_is_page_section", true) == "yes");
    	if ( $section_page && ( $post_id != $main_page_id ) ) {

    		if (get_post_meta($post_id, "mc_page_section", true) == "home")
      	    	get_template_part('sections/home_section');

      	    if (get_post_meta($post_id, "mc_page_section", true) == "default")
      	    	get_template_part('sections/normal_section');	

      	    if (get_post_meta($post_id, "mc_page_section", true) == "portfolio")
      	    	get_template_part('sections/portfolio_section');	
      	    	
    		if (get_post_meta($post_id, "mc_page_section", true) == "parallax")
      	    	get_template_part('sections/parallax_section');

    		if (get_post_meta($post_id, "mc_page_section", true) == "parallaxclassic")
      	    	get_template_part('sections/parallax_section2');
      	    	
    		if (get_post_meta($post_id, "mc_page_section", true) == "contact")
