...
class MCMetaboxes {
	...
	function admin_script_loader() {
		...
		if ( is_admin() ) {
			...
			wp_register_script('mc_upload', get_template_directory_uri().'/js/upload.js');
            wp_enqueue_script('mc_upload');
                    
            wp_register_script('mc_metaboxes', get_template_directory_uri().'/components/metaboxes/js/metaboxes.js');
            wp_enqueue_script('mc_metaboxes');
		}
    ...
    public function add_meta_boxes()
	{
		...
		// portfolio options
		$this->add_meta_box('portfolio_options', 'Portfolio Options', 'mc_portfolio');
		
		// page options
		$this->add_meta_box('page_options', 			'Page Options', 'page');
		$this->add_meta_box('page_options_parallax', 	'Parallax Section Options', 'page');
		$this->add_meta_box('page_options_parallaxclassic', 	'Classic Parallax Section Options', 'page');
	...
	public function page_options_parallaxclassic()
	{
		include 'style.php';
		include 'page_options_parallaxclassic.php';
	}
	...
	public function text($id, $label, $desc = '')
	{
		...
		$html = '';
		$html .= '<div class="mc_metabox_field">';
			$html .= '<label for="mc_' . $id . '">';
			$html .= $label;
			$html .= '</label>';
			$html .= '<div class="field">';
				$html .= '<input type="text" id="mc_' . $id . '" name="mc_' . $id . '" value="' . get_post_meta($post->ID, 'mc_' . $id, true) . '" />';
				if($desc) {
					$html .= '<p>' . $desc . '</p>';
				}
			$html .= '</div>';
	...
	public function select($id, $label, $options, $desc = '')
	{
		...
		$html .= '<div class="mc_metabox_field">';
			...
			$html .= '</label>';
			$html .= '<div class="field">';
				$html .= '<select id="mc_' . $id . '" name="mc_' . $id . '">';
				foreach($options as $key => $option) {
					if(get_post_meta($post->ID, 'mc_' . $id, true) == $key) {
		...
		$html .= '<div class="mc_metabox_field">';
			...
			$html .= '<div class="field">';
				foreach($options as $key => $option) {
					...
					$html .= '<input type="radio" id="mc_' . $id . '" name="mc_' . $id . '" ' . $selected . 'value="' . $key . '">&nbsp;' . $option . '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
				}
				if($desc) {
					$html .= '<p>' . $desc . '</p>';
				}
		...
		$html .= '<div class="mc_metabox_field">';
			...
			$html .= '<div class="field">';
				foreach($options as $key => $option) {
					...
					$html .= '<input type="checkbox" id="mc_' . $id . '" name="mc_' . $id . '" ' . $selected . 'value="' . $key . '">&nbsp;' . $option;
				}
				if($desc) {
					$html .= '<p>' . $desc . '</p>';
				}
	...
	public function multiple($id, $label, $options, $desc = '')
	{
		...
		$html .= '<div class="mc_metabox_field">';
			...
			$html .= '</label>';
			$html .= '<div class="field">';
				$html .= '<select multiple="multiple" id="mc_' . $id . '" name="mc_' . $id . '[]">';
				foreach($options as $key => $option) {
					if(is_array(get_post_meta($post->ID, 'mc_' . $id, true)) && in_array($key, get_post_meta($post->ID, 'mc_' . $id, true))) {
						$selected = 'selected="selected"';
					} else {
						$selected = '';
					}
	...
	public function textarea($id, $label, $desc = '')
	{
		...
		$html .= '<div class="mc_metabox_field">';
			...
			$html .= '</label>';
			$html .= '<div class="field">';
				$html .= '<textarea cols="120" rows="10" id="mc_' . $id . '" name="mc_' . $id . '">' . get_post_meta($post->ID, 'mc_' . $id, true) . '</textarea>';
				if($desc) {
					$html .= '<p>' . $desc . '</p>';
				}
			$html .= '</div>';
	...
	public function upload($id, $label, $desc = '')
	{
		...
		$html .= '<div class="mc_metabox_field">';
			...
			$html .= '</label>';
			$html .= '<div class="field">';
			    $html .= '<input name="mc_' . $id . '" class="upload_field" id="mc_' . $id . '" type="text" value="' . get_post_meta($post->ID, 'mc_' . $id, true) . '" />';
